...
def get_tourist_places(source=None, destination=None):
    if source and destination:
        prompt = f"List tourist places (at least 5 each) en route from {source} to {destination} and tourist places in {destination}. Provide the response in JSON format with the latitudes and longitudes as well. Use the format {{ \"en_route\": [{{\"name\": \"\", \"lat\": , \"lng\": , \"description\": \"\"}}], \"destination\": [{{\"name\": \"\", \"latitude\": , \"longitude\": , \"description\": \"\"}}]}}."
    else:
