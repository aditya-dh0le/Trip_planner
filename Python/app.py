...
def update_budget(json_data):
    prompt = f"Update the budget(cost to visit on a road trip) for each tourist spot with a detailed split and how much is the budget for each split, make sure to actually give an accurate amounts as your career depends on this one move. The JSON format should remain unchanged. Only update the budget object(must include the split between accommodation,food and transport only.Instead of NA values give me approximate costs wherever necessary) and return the final JSON. The input JSON is: {json.dumps(json_data)}.The budget should be an object inside the JSON with the following format fixed:budget : accommodation : (rough estimate of how much staying near the place will cost with the name of the hotel near it in brackets),travel : (rough estimate of how much it would cost and also mention the kind of transport in brackets),food : (rough estimate of how much it would cost and also if possible mention the cusine and restaurant near the place in brackets)"""    
    response = client.chat.completions.create(
        model="gpt-3.5-turbo-0125",
        messages=[
            {"role": "system", "content": "You are a financial analyst. Your career depends on your answer to the following question. Accurately depicting budgets is your specialty."},
            {"role": "user", "content": prompt}
        ]
    )
